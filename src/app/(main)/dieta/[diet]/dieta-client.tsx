...
export default function DietaClient({ activeQ, activeCat, activeTiempo, activeCount }: Props) {
  ...
  function buildUrl(overrides: { q?: string; cat?: string; tiempo?: string }) {
  ...
  function toggleTiempo(key: string) {
    startTransition(() => router.push(buildUrl({ tiempo: activeTiempo === key ? '' : key })))
  }
        ...
        <button onClick={() => setShowFilters(true)}
          ...
          {hasFilters && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold text-white"
              ...
              {(activeCat ? 1 : 0) + (activeTiempo ? 1 : 0)}
            </span>
          )}
        </button>
      </div>
        ...
        <div className="flex flex-wrap gap-2 mt-2.5 items-center">
          ...
          {activeTiempo && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium"
              ...
              <button onClick={() => toggleTiempo(activeTiempo)}>×</button>
            </span>
          )}
                  ...
                  {TIMES.map(t => {
                    ...
                    return (
                      <button key={t.key} onClick={() => toggleTiempo(t.key)}
                        ...
                        <span className={`text-[13px] font-semibold leading-tight`} style={{ color: active ? 'var(--brown-900)' : 'var(--brown-700)' }}>{t.label}</span>
                      </button>
                    )
                  })}
                </div>

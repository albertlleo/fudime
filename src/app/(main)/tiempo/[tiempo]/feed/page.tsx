...
export default async function TiempoFeedPage({
  params,
  searchParams,
}: {
  ...
  const [likedResult, savedResult, commentCountsResult, likeCountsResult] = await Promise.all([
    user ? supabase.from('likes').select('recipe_id').eq('user_id', user.id).in('recipe_id', recipeIds) : Promise.resolve({ data: [] }),
    user ? supabase.from('saves').select('recipe_id').eq('user_id', user.id).in('recipe_id', recipeIds) : Promise.resolve({ data: [] }),
  ...
  for (const row of commentCountsResult.data ?? []) commentCountMap[(row as any).recipe_id] = (commentCountMap[(row as any).recipe_id] ?? 0) + 1
  const likeCountMap: Record<string, number> = {}
  for (const row of likeCountsResult.data ?? []) likeCountMap[(row as any).recipe_id] = (likeCountMap[(row as any).recipe_id] ?? 0) + 1

  return (
    <CreatorFeed recipes={recipes} likedIds={likedIds} savedIds={savedIds}
      likeCountMap={likeCountMap} commentCountMap={commentCountMap}
      userId={user?.id ?? null} initialIndex={initialIndex} />
  )
}

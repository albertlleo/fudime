    ...
    <div ref={containerRef} className="h-dvh snap-start snap-always relative flex-shrink-0 overflow-hidden"
      ...
      {doubleTapHeart && (
        <div key={doubleTapHeart.key} className="pointer-events-none absolute z-20"
          ...
          <svg viewBox="0 0 24 24" fill="#f59e0b" className="w-24 h-24 drop-shadow-2xl">
      ...
      {/* Bottom info */}
      <div className="absolute bottom-32 lg:bottom-4 left-0 right-16 px-4 pb-2" onClick={e => e.stopPropagation()}>
      ...
      {/* Right actions */}
      <div className="absolute bottom-36 lg:bottom-8 right-3 flex flex-col items-center gap-6" onClick={e => e.stopPropagation()}>
        {isOwner && (
        ...
        <button onClick={handleLike} className="flex flex-col items-center gap-1.5 active:opacity-80">
          <div className={`transition-transform duration-200 ${likeAnim ? 'scale-[1.4]' : 'scale-100'}`}>
            <svg viewBox="0 0 24 24" fill={isLiked ? '#f59e0b' : 'white'} stroke="none" className="w-7 h-7 drop-shadow-lg">
              <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" />
            </svg>
          </div>
